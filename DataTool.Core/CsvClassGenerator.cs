...
public static class CsvClassGenerator
{
    ...
    public static string GeneratePreviewFromXlsxFast(string xlsxPath, int maxRows = 64)
    {
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine($"    public partial class {className}Container : DataContainerBase<{className}>");
        sb.AppendLine("    {");
        sb.AppendLine(IfUniTask);
    ...
    public static string GenerateTableContainerFile(string csvPath, string classNameOverride = null) =>
        GenerateTableContainerFile(CsvTableParser.Parse(csvPath, classNameOverride));

    public static string GenerateTableContainerFile(CsvTableParseResult table)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated from CSV — 가공: 같은 이름의 partial 파일을 별도로 추가>");
        sb.AppendLine("using MessagePack;");
        sb.AppendLine("using System.Collections.Generic;");
        sb.AppendLine(IfUniTask);
        sb.AppendLine("using Cysharp.Threading.Tasks;");
