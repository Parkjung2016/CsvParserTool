        ...
        private static Bitmap TryCreateBlurredSnapshot(Form owner)
        {
            ...
            try
            {
                using (var source = new Bitmap(clientBounds.Width, clientBounds.Height, PixelFormat.Format32bppPArgb))
                {
                    ...
                    int smallWidth = Math.Max(1, clientBounds.Width / 12);
                    int smallHeight = Math.Max(1, clientBounds.Height / 12);
                    var reduced = new Bitmap(smallWidth, smallHeight, PixelFormat.Format32bppPArgb);
                    using (Graphics down = Graphics.FromImage(reduced))
                    {
                        down.CompositingMode = CompositingMode.SourceCopy;
                        down.InterpolationMode = InterpolationMode.Bilinear;
                        down.PixelOffsetMode = PixelOffsetMode.Half;
                        ...
                        using (var tintBrush = new SolidBrush(tint))
                            down.FillRectangle(tintBrush, 0, 0, smallWidth, smallHeight);
                    }
                    return reduced;
                }
            }
